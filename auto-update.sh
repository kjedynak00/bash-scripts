...
notify(){
    ...
    curl -s -X POST -H 'Content-type: application/json' \
        --data "$(jq -nc --arg msg "$MESSAGE on $HOSTNAME" '{"content": $msg}')" \
...
if [ -z "$NEW_PACKAGES" ]; then
    ...
    if [ "$EXIT_CODE" -eq 0 ]; then
        notify "OK" "System updated successfully. Packages: $PKG_LIST"
    elif [ "$EXIT_CODE" -eq 1 ]; then
        notify "FAIL" "apt-get update failed. Packages pending: $PKG_LIST"
    elif [ "$EXIT_CODE" -eq 2 ]; then
        notify "FAIL" "apt-get upgrade failed. Packages pending: $PKG_LIST"
    else
        notify "FAIL" "Update failed with unknown error. Packages pending: $PKG_LIST"
    fi
fi
